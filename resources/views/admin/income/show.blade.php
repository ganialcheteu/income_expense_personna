...
@section('dashboard_body_content')
    ...
    <div class="showRessource">
        {{-- ----------------------------- incomeS ----------------------------- --}}
        <?php
            //No image for this income
            if (!$income->images()->exists()) {
                echo "
                                            <div class=\"noImageText  mb-2\">
                                             No Image Available For Income:
                                             '<span>htmlspecialchars($income->name, ENT_QUOTES, 'UTF-8')</span>'
                                            </div>
                                            ";
            }
         ?>
        <div id="owl" class="owl-carousel owl-theme mx-auto mb-2">
            @foreach ($income->images as $image)
                <div class="item">
                    <img src="{{ asset('storage/' . $image->path) }}" alt="{{ $income->name }}" id="currentImage"
        ...
        {{-- ----------------------------- ABOUT income-------------------------- --}}
        <div id="incomeDescription">
            <div>
                <span class="content">{{ $income->name }}</span>
            </div>
            <div>
                <x-go-back-button />
            </div>
    ...
    {{-- fullscreen modal --}}
    <div class="modal fade bg-info" id="ModalFullscreen" tabindex="-1" aria-labelledby="{{ $income->name }} Image"
        aria-modal="true" role="dialog">
        <div class="modal-dialog modal-fullscreen">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-4" id="ModalFullscreenLabel">{{ $income->name }}</h1>
