    ...
    <div class="col-md-7 grid-margin stretch-card mx-auto">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" method="POST" action="{{ route('incomes/income_store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                placeholder="Enter income Name" value="{{ old('name') }}" name="name" required>
                            @error('name')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount"
                                placeholder="Enter income Amount in FRS CFA" name="amount" value="{{ old('amount') }}"
                                min="1" required>
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="date" class="form-control @error('payment_date') is-invalid @enderror"
                                id="payment_date" placeholder="Enter income Date" value="{{ old('payment_date', date('Y-m-d')) }}"
                                name="payment_date" required title="Click on calendar, choose income date">
                            @error('payment_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select
                                ...
                                @foreach ($incomeTypes as $incomeType)
                                    <option value="{{ $incomeType->id }}"
                                        {{ old('income_type_id') == $incomeType->id ? 'selected' : '' }}>
                                        {{ $incomeType->type }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select
                                ...
                                @foreach ($incomeCategories as $incomeCategory)
                                    <option value="{{ $incomeCategory->id }}"
                                        {{ old('income_category_id') == $incomeCategory->id ? 'selected' : '' }}>
