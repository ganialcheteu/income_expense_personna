    ...
    <div class="col-md-7 grid-margin stretch-card mx-auto">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" method="POST" action="{{ route('expenses/expense_store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                placeholder="Enter Expense Name" value="{{ old('name') }}" name="name" required>
                            @error('name')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="number" class="form-control @error('amount') is-invalid @enderror" id="amount"
                                placeholder="Enter Expense Amount in Frs CFA" name="amount" value="{{ old('amount') }}"
                                min="1" required>
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="date" class="form-control @error('payment_date') is-invalid @enderror"
                                id="payment_date" placeholder="Enter Expense Date" value="{{ old('payment_date',date('Y-m-d')) }}"
                                name="payment_date" required title="Click on calendar, choose expense date">
                            @error('payment_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select
                                ...
                                @foreach ($expenseTypes as $expenseType)
                                    <option value="{{ $expenseType->id }}"
                                        {{ old('expense_type_id') == $expenseType->id ? 'selected' : '' }}>
                                        {{ $expenseType->type }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select
                                ...
                                @foreach ($expenseCategories as $expenseCategory)
                                    <option value="{{ $expenseCategory->id }}"
                                        {{ old('expense_category_id') == $expenseCategory->id ? 'selected' : '' }}>
